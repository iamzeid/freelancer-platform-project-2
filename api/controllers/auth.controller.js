...
const handleErrors = (err) => {
  ...
  if (err.message === "Passwords do not match") { // Add condition for passwords not matching
...
export const register = async (req, res) => {
  if (req.body.googleAccessToken) {
    ...
    try {
      const { email, password, retypePassword, username, role, picture, bio } = req.body;
      ...
      return res.status(201).json({ message: "User registered successfully", redirectTo: "/verification" });
